...
void copy_with_kernel_impl(sycl::queue& q, const copy_spec& spec, IdxType preferred_wg_size, sycl::event &event) {
	...
	if(spec.source_layout.fragment_count == spec.target_layout.fragment_count) {
		...
		if(frag_elems == 1) {
			if(tgt_stride == 1) {
				event = q.parallel_for(ndr, [=]([[maybe_unused]] sycl::nd_item<1> idx) { //
...
template <typename T>
void copy_with_kernel_impl(sycl::queue& q, const copy_spec& spec, int32_t preferred_wg_size, sycl::event& event) {
	...
	if(spec.source_layout.fragment_count < max && spec.source_layout.fragment_count < max && spec.source_layout.effective_stride() < max
		...
		copy_with_kernel_impl<T, int32_t>(q, spec, preferred_wg_size, event);
	} else {
		copy_with_kernel_impl<T, int64_t>(q, spec, preferred_wg_size, event);
	}
}
...
void copy_with_kernel(sycl::queue& q, const copy_spec& spec, int32_t preferred_wg_size, sycl::event& event) {
	...
	if(smaller_fragment_size % sizeof(sycl::int16) == 0 && smaller_stride % sizeof(sycl::int16) == 0) {
		copy_with_kernel_impl<sycl::int16>(q, spec, preferred_wg_size, event);
	} else if(smaller_fragment_size % sizeof(sycl::int8) == 0 && smaller_stride % sizeof(sycl::int8) == 0) {
		copy_with_kernel_impl<sycl::int8>(q, spec, preferred_wg_size, event);
	} else if(smaller_fragment_size % sizeof(sycl::int4) == 0 && smaller_stride % sizeof(sycl::int4) == 0) {
		copy_with_kernel_impl<sycl::int4>(q, spec, preferred_wg_size, event);
	} else if(smaller_fragment_size % sizeof(sycl::int2) == 0 && smaller_stride % sizeof(sycl::int2) == 0) {
		copy_with_kernel_impl<sycl::int2>(q, spec, preferred_wg_size, event);
	} else if(smaller_fragment_size % sizeof(int32_t) == 0 && smaller_stride % sizeof(int32_t) == 0) {
		copy_with_kernel_impl<int32_t>(q, spec, preferred_wg_size, event);
	} else if(smaller_fragment_size % sizeof(int16_t) == 0 && smaller_stride % sizeof(int16_t) == 0) {
		copy_with_kernel_impl<int16_t>(q, spec, preferred_wg_size, event);
	} else {
		copy_with_kernel_impl<int8_t>(q, spec, preferred_wg_size, event);
	}
}
